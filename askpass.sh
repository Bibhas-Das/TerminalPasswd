...
# Next Edit       : 18 Aug 2024, 15:59 PM        [ Auto Lock feature using background excecutable program ]
# Last Edit       : 28 Feb 2025, 05:00 PM		 [ Bug fixed : autosetup && trusted wifi searchhing ]
...
__setup__()
{
	...
	if [ -e "$p_hash" ]
		then
			debug "The password file is exixts"
			# Check if the file is empty
			if [ ! -s "$p_hash" ] 
			then
				debug "The password file is empty"
				rm $p_hash
				debug "GOing to run the program from begginging..."
				sleep 5
				__setup__
			fi
			#else all is good
			debug "All setup is already done"
	else
		...
		#if [ -e "$HOME/.bashrc" ]
		#then
		#	debug "Trying to append execuatable text on $HOME/.bashrc"
		...
		if [ -e "$HOME/.bashrc" ]
		then
			debug "Checking if executable text is already present in $HOME/.bashrc"

			# Check if the export path line is already present
			if ! grep -qx "export PATH=\$PATH:$secrectDir/commands" "$HOME/.bashrc"; then
				echo "export PATH=\$PATH:$secrectDir/commands" >> "$HOME/.bashrc"
				debug "Added export PATH line to .bashrc"
			else
				debug "export PATH line already exists in .bashrc, skipping"
			fi

			# Check if ./.askpass line is already present
			if ! grep -qx "$HOME/.askpass" "$HOME/.bashrc"; then
				echo "$HOME/.askpass" >> "$HOME/.bashrc"
				debug "Added .askpass line to .bashrc"
			else
				debug ".askpass line already exists in .bashrc, skipping"
			fi
		fi
		...
		#if [ -e "$HOME/.zshrc" ]
		#then
		#	debug "Trying to append execuatable text on $HOME/.zshrc"
		...
		if [ -e "$HOME/.zshrc" ]
		then
			debug "Checking if executable text is already present in $HOME/.zshrc"

			# Check if the export path line is already present
			if ! grep -qx "export PATH=\$PATH:$secrectDir/commands" "$HOME/.zshrc"; then
				echo "export PATH=\$PATH:$secrectDir/commands" >> "$HOME/.zshrc"
				debug "Added export PATH line to .zshrc"
			else
				debug "export PATH line already exists in .zshrc, skipping"
			fi

			# Check if ./.askpass line is already present
			if ! grep -qx "$HOME/.askpass" "$HOME/.zshrc"; then
				echo "$HOME/.askpass" >> "$HOME/.zshrc"
				debug "Added .askpass line to .zshrc"
			else
				debug ".askpass line already exists in .zshrc, skipping"
			fi
		fi
		...
		#if [ -e "$HOME/.fishrc" ]
		#then
		#	debug "Trying to append execuatable text on $HOME/.fishrc"
		...
		if [ -e "$HOME/.fishrc" ]
		then
			debug "Checking if executable text is already present in $HOME/.fishrc"

			# Check if the export path line is already present
			if ! grep -qx "export PATH=\$PATH:$secrectDir/commands" "$HOME/.fishrc"; then
				echo "export PATH=\$PATH:$secrectDir/commands" >> "$HOME/.fishrc"
				debug "Added export PATH line to .fishrc"
			else
				debug "export PATH line already exists in .fishrc, skipping"
			fi

			# Check if ./.askpass line is already present
			if ! grep -qx "$HOME/.askpass" "$HOME/.fishrc"; then
				echo "$HOME/.askpass" >> "$HOME/.fishrc"
				debug "Added .askpass line to .fishrc"
			else
				debug ".askpass line already exists in .fishrc, skipping"
			fi
		fi
		...
		create_password_hash_file
		debug "The files, folders, setup is done"
	fi
}
